...
&emsp;&emsp;&emsp;Bedroom - 2(min) - 4(max)  
&emsp;&emsp;&emsp;Price - £140,000(min) - £350,000(max)  
...
## Milestone 3
Prototype finding the individual page for each entry
### Task 1:
Create a Scraper class  
&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;This class will contain all methods used to scrape data from the  website- [Zoopla](https://www.zoopla.co.uk/).
### Task 2:
...
- Create **__search_data()** method to navigate the home page of [Zoopla](https://www.zoopla.co.uk/).
...
> Note:
Please replace the respective driver as per OS in the path of the project directory - **./drivers** : Chrome Driver [Version 107.0.5304.62](https://chromedriver.storage.googleapis.com/index.html?path=107.0.5304.62/)
Firefox Driver [Version 0.32.0](https://github.com/mozilla/geckodriver/releases)
### Task 4:
...
```docker build --file Dockerfile . -t datacollectionpipeline```

### Run the container to generate data
```docker run -it --shm-size="1g" -v path/to/save/data/host:/root/datacollectionpipeline/raw_data datacollectionpipeline /bin/bash```
>Note: Replace this **path/to/save/data/host** with your own path on host machine to save data.
